...
int main()
{
    ...
    {
        Rect rect;
        // код вызывает неявный конструктор по умолчанию, сгенерированный компилятором, который не инициализирует поля (там будут случайные значения). В конце вызывается неявный деструктор, сгенерированный компилятором, который ничего не делает для простых типов, а для сложных вызывает их деструктор
    } // destructor
    printf("-----------------\n");
    ...
    {
        Rect rect(1, 2, 3, 4);
        // Rect rect(); //ошибка, тк не был объявлен конструктор по умолчанию
    } // destructor
    printf("-----------------\n");
    ...
    {
        Rect rect1(1, 2, 3, 4); // полный с 4 параметрами
        Rect rect2 = rect1;     // копирует
        Rect rect3(rect1);      // Rect rect4 = Rect(rect1); то же самое
        // в общем случае копирует все поля аргумента в поля
    } // destructor 3-2-1
    printf("-----------------\n");
    /**
...

    {
        ...
        // printf("%d\n", r1.get_bottom());
        Rect *pR = new Rect(1, 2, 1, 2); // конструктор для всех полей
        {
            Rect r2(r1);    // конструктор-копирование
            Rect arRect[2]; // дважды конструктор по умолчанию в массиве
            for (int i = 0; i < 3; i++)
            {
                static Rect r3(i, i, i, i); // конструктор для всех полей
                Rect r4(*pR);               // конструктор-копирование
                Rect r5(i, i, i, i);        // конструктор для всех полей
            } // деструктор для r4, r5 (после каждой итерации, всего 3 раза), для r3 не вызывается, тк он статичный
        }
        delete pR; // деструктор pR
        // деструктор r1
        // деструктор r3 будет в конце выполнения программы в порядке от последнего к первому static
    }
    printf("-----------------\n");
    ...
    {
        Rect r1;
        // std::cout << r1.left << std::endl; //‘int Rect::left’ is private within this context. left приватный, к нему нельзя обратиться напрямую. только через геттеры и сеттеры
    }
    printf("-----------------\n");
    ...
    {
        Rect r;
        cout << r.get_left() << "; " << r.get_right() << "; " << r.get_top() << "; " << r.get_bottom() << '\n';
        r.set_all(1, 2, 3, 4);
        cout << r.get_left() << "; " << r.get_right() << "; " << r.get_top() << "; " << r.get_bottom() << '\n';
    }
    printf("-----------------\n");
     ...
     *
     * функция-член "привязана" к классу, а обычная фукнция независима от него. для ФЧ всё вращается вокруг объекта класса, она имеет доступ к его полям, может изменять их напрямую, в отличие от обычной функции.
     */
...

    {
        ...
        cout << "\nИсходный прямоугольник:" << endl;
        print_rect(rect);

        rect.move(1, 2);
        cout << "\nПосле rect.move(1, 2):" << endl;
        print_rect(rect);

        rect.move(1);
        cout << "\nПосле rect.move(1):" << endl;
        print_rect(rect);

        rect.move();
        cout << "\nПосле rect.move()" << endl;
        print_rect(rect);

        rect.move(-5, -3);
        cout << "\nПосле rect.move(-5, -3):" << endl;
        print_rect(rect);
    }
    printf("-----------------\n");
    ...
    {
        Rect r1(0, 10, 10, 0), r2(10, 9, 10, -3); // r1 -> r2
        Rect r3 = bounding_rect(r1, r2);          // Constructor_copy для r2 -> r1, Constructor_four для r3 в функции
        print_rect(r3);
    } // деструкторы r3 -> r2 -> r1
    printf("-----------------\n");
